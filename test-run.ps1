...
foreach($wiki in $wikis)
{
    ...
    # run the converter
    $output = dotnet run --project ".\AzureDevOps.WikiPDFExport\AzureDevOps.WikiPDFExport.csproj" -- -p $wiki.FullName -o ".\tests\$($wiki.Name).pdf"
    ...
    # persist the time for later comparison
    $obj = [PSCustomObject]@{
        Name     = $wiki.Name
        Hours    = $hours
        Minutes  = $minutes
