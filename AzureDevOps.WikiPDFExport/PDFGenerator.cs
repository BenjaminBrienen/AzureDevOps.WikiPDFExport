...
#endif
	{
		...
		if (string.IsNullOrEmpty(chromePath))
		{
			...
			var fetcherOptions = new BrowserFetcherOptions
			{
				...
				// TODO
				// CustomFileDownload = async (address, filename) => {},
			};
			...
			|| !string.IsNullOrEmpty(_options.FooterTemplatePath))
		{
			...
			{
				headerTemplate = await File.ReadAllTextAsync(_options.HeaderTemplatePath).ConfigureAwait(false);
			}
			...
			{
				footerTemplate = await File.ReadAllTextAsync(_options.FooterTemplatePath).ConfigureAwait(false);
			}
